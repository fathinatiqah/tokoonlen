            ...
            <div class='card-body'>
                <table class="table table-bordered fs-5 fw-light text-center">
                    ...
                    if (isset($_POST["cari"])) {
                        ...
                        $cari=$_POST['cari'];
                        $query_transaksi= mysqli_query($conn,"SELECT * from transaksi where id_transaksi like '%$cari%' or id_register like '%$cari%' or tanggal_transaksi like '%$cari%'");
                    }else{
                        //jika tidak ada keyword pencarian
                        $query_transaksi= mysqli_query($conn,"SELECT * from transaksi join register on register.id_register= transaksi.id_register ORDER BY id_transaksi DESC;") or die (mysqli_error($conn));
                    }
                        $no=0;
