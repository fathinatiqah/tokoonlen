<?php
    ...
    if (count($cart) > 0) {
        ...
        $query_transaksi = mysqli_query($conn, "INSERT INTO transaksi (id_register, tgl_transaksi, tgl_datang, status)
        VALUES ('".$_SESSION['id_register']."', '".$tgl_transaksi."','".$tgl_tiba."', 'new')");
        mysqli_error($conn);
        if ($query_transaksi) {
